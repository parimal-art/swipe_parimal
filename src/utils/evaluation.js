...
// Long-answer evaluation (existing logic)
...
// MCQ evaluation - returns full marks if correct, 0 if wrong
export function evaluateMCQAnswer(selectedOption, correctAnswer, maxScore = 10) {
  ...
  return {
    isCorrect,
    score,
    matchedKeywords: isCorrect ? ['correct_answer'] : [],
    selectedOption,
    correctAnswer,
  };
}
...
export function selectQuestions(allQuestions) {
  ...
  const longQuestions = allQuestions.filter(q => (q.questionType || 'long') === 'long');
  const easy = longQuestions.filter(q => q.difficulty === 'easy');
  const medium = longQuestions.filter(q => q.difficulty === 'medium');
  const hard = longQuestions.filter(q => q.difficulty === 'hard');
  const shuffle = (array) => {
